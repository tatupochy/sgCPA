...
d.addEventListener('DOMContentLoaded', () => {
    ...
    if(url.includes('editar_alumno')) return;
    ...
    countrySelect.addEventListener('change', async(e) => {
        ...
        const url = `/obtener_ciudades_por_pais/${value}`
        const res = await fetch(url)
        const json = await res.json()
        const cities = json.ciudades;
        if(cities.length == 0) {
            citiesSelect.innerHTML = '<option value="">Pais Sin Ciudad</option>';
            ...
            citiesSelect.innerHTML = '<option value="">Seleccionar Ciudad</option>';
            citiesSelect.append(fragment)       
